...
{
	[Route("v{version:apiVersion}/orders/chefsexperience")]
	public class ChefsExperienceOrdersController : ApiBaseController
	{
		private readonly IBackgroundTaskQueue _taskQueue;

		public ChefsExperienceOrdersController(IBackgroundTaskQueue taskQueue)
		{
		...
		private void QueueOrderProcessingAsync(HttpOrderRequestDto newOrderDto)
		{
			foreach (var orderItem in newOrderDto.OrderItems)
			{
				_taskQueue.QueueBackgroundWorkItem(async token =>
				{
					...
					var secondsRequiredForProcessing = rnd.Next(1, 5); // between 5 and 15 seconds
